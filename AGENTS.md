...
To ensure consistent and effective search engine optimization, please adhere to the following guidelines when updating the application.

*   **Canonical URL**: The canonical URL is set in `index.html` and should not be changed. It is `https://PortablePrinterTTRPGCardGenerator.BagsOfFolding.com/`.
*   **Meta Description**: The meta description in `index.html` should be a concise summary of the application's purpose. It needs to be synchronized with the Open Graph and Twitter descriptions.
*   **Social Media Cards (Open Graph & Twitter)**:
    *   The `og:title` and `twitter:title` should match the page's main `<title>`.
    *   The `og:description` and `twitter:description` should match the main `meta name="description"`.
    *   The `og:image` and `twitter:image` should point to a full, absolute URL of an image that is visually representative of the application.
    *   The `og:image:alt` and `twitter:image:alt` tags should provide descriptive alt text for the image.
    *   The `og:image:width` and `og:image:height` tags should be set to standard dimensions for social media cards (e.g., 1200x630).
*   **Structured Data**: The `index.html` page includes a `<script type="application/ld+json">` for `WebApp` schema. If core details about the application change (like its name or primary screenshot), this data should be updated accordingly.
