...
A meta description is an HTML attribute that provides a brief summary of a web page. Search engines often display the meta description in search results, which can influence click-through rates. The meta description needs to be synchronized with the Open Graph and Twitter descriptions.
...
* The og:image and twitter:image should point to a full, absolute URL of an image that is visually representative of the page.
...
### Structured Data
Include a <script type="application/ld+json"> for WebApp schema. If core details about the application change (like its name or primary screenshot), this data should be updated accordingly.
